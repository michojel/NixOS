...
let
  userIdentityFile = config.users.extraUsers."${config.local.username}".home + "/.ssh/id_rsa_nopw";
  rootIdentityFile = "/root/.ssh/id_rsa_nopw";
  sshRootPath = "/ssh";
  mkMountPoint = { user, host }: sshRootPath + "/" + (if user == "${config.local.username}" then "" else user + "-") + host;
